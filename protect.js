...
(function(){
    ...
    /* ==============================
       4. SECURE SESSION
       FIX: Old v3 used only
       localStorage which anyone
       could set to "1" in DevTools.
       Now BOTH localStorage AND
       sessionStorage must match,
       making manual spoofing harder.
    ============================== */
    function _saveSession(key){
        try{
            const token = _H(key + "sn_salt_v4");
    ...
    function _checkSession(){
        try{
            ...
            // Token must match one of the real keys
            for(let rk of ["SN-29XK-8D2P","SN-AB12-X9Q7"]){
                if(_H(rk + "sn_salt_v4") === ls) return true;
            }
            return false;
        }catch(e){
            return false;
    ...
    /* ==============================
       🔐 LOCK
       FIX: No longer exposed as
       window._LOCK_APP — kept
       fully private inside closure
    ============================== */
    function _lock(){
        _unlocked = false;
    ...
    /* ==============================
       6. POPUP UI
       FIX: Replaced ugly prompt()
       with a styled modal popup
    ============================== */
    ...
    /* ==============================
       7. DEVTOOLS CHECK
       FIX: Now immediately locks
       on high threat score instead
       of just incrementing counter
    ============================== */
    function _dtCheck(){
        const t = 160;
        if(
    ...
    function _init(){
        ...
        // Valid session from a previous license key entry
        if(_checkSession()){
            _unlocked = true;
            return;
        }
